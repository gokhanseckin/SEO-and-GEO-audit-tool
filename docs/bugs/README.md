...
| [BUG-012](BUG-012-indexed-pages-capped-at-10.md) | Medium | Backend / UI | Indexed pages estimate pinned at 10 (reads `organic.length` instead of `searchInformation.totalResults`) | Fixed in `6eabe7c` |
| [BUG-013](BUG-013-silent-fire-and-forget-dispatch.md) | High | Backend / API | Silent fire-and-forget Edge Function dispatch strands audits in `status='running'` when gateway rejects | Fixed (pending commit) |
